...
- 本次检查时间：**2026-07-22T17:48:16+08:00**
...
|---|---|---|---:|---:|---|
| node01 | Q50 | 实时快照（运行中） | 126.0 / 1500 | 3/17 | 重度 |
| node04 | Q400 -> Q300 | 实时快照（运行中） | 511.5 / 1500 | 14/17 | 重度 |
| node05 | Q200 -> Q100 | 实时快照（运行中） | 532.5 / 1500 | 11/17 | 重度 |
...
状态：**实时快照（运行中）**；数据截止 **126.0 s**；严格重度毁伤 **3/17**；PDF毁伤树整机等级 **重度**。

| 材料/设备 | 毁伤等级 | 最高温度（C） | 重度阈值连续时间（s） | 重度要求（s） | 判定 |
|---|---|---:|---:|---:|---|
| RADM | 未毁伤 | 737.4 | 106.5 | 180.0 | 峰值或持续时间不足 |
| WINS | 重度 | 950.3 | 124.5 | 8.0 | 达到重度 |
| BED | 未毁伤 | 267.1 | 0.0 | 5.0 | 峰值或持续时间不足 |
| CURT | 轻度 | 343.4 | 0.0 | 5.0 | 峰值或持续时间不足 |
| U4 | 未毁伤 | 146.6 | 0.0 | 5.0 | 峰值或持续时间不足 |
| SEAT | 重度 | 2726.8 | 124.5 | 5.0 | 达到重度 |
| AL2024 | 重度 | 839.7 | 103.5 | 60.0 | 达到重度 |
| AL5052 | 未毁伤 | 324.6 | 0.0 | 60.0 | 峰值或持续时间不足 |
| AL7075 | 未毁伤 | 162.6 | 0.0 | 60.0 | 峰值或持续时间不足 |
| O2TANK | 未毁伤 | 117.0 | 0.0 | 60.0 | 峰值或持续时间不足 |
| H1 | 未毁伤 | 78.3 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H2 | 未毁伤 | 78.4 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H3 | 未毁伤 | 198.2 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H4 | 未毁伤 | 158.4 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H5 | 未毁伤 | 62.7 | 0.0 | 180.0 | 峰值或持续时间不足 |
...
当前峰值为 **737.4 C**，重度判据为 **400 C 持续 180 s**，实际最长连续时间为 **106.5 s**。
...
峰值已经超过重度温度阈值，但还缺少 **73.5 s** 连续保持时间。这属于持续时间不足，而不是峰值温度不足；后续若温度重新升高并连续越过阈值，等级仍可能改变。
...
当前峰值为 **324.6 C**，重度判据为 **400 C 持续 60 s**，实际最长连续时间为 **0.0 s**。
...
峰值仍低于重度阈值 **75.4 C**，且有效探针没有直接受照面。其升温主要来自邻近火焰、热烟气和舱内辐射，因此结果受二次火灾位置与持续性控制。

##### AL7075：铝合金7075

当前峰值为 **162.6 C**，重度判据为 **400 C 持续 60 s**，实际最长连续时间为 **0.0 s**。
...
峰值仍低于重度阈值 **237.4 C**。对象只有部分监测面直接受照，未受照部分及内部导热共同削弱局部脉冲升温，脉冲后散热又使温度回落。

##### O2TANK：铝合金7075氧气瓶壁

当前峰值为 **117.0 C**，重度判据为 **400 C 持续 60 s**，实际最长连续时间为 **0.0 s**。
...
峰值仍低于重度阈值 **283.0 C**。对象只有部分监测面直接受照，未受照部分及内部导热共同削弱局部脉冲升温，脉冲后散热又使温度回落。
...
当前峰值为 **198.2 C**，重度判据为 **400 C 持续 5 s**，实际最长连续时间为 **0.0 s**。

单位面积热容量为 `rho*cp*d = 2700*896*0.003 = 7258 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `275.8 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **0/14** 个对应正外部热流面，登记峰值为 **0 kW/m2**。标称 Q=50 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值仍低于重度阈值 **201.8 C**，且有效探针没有直接受照面。其升温主要来自邻近火焰、热烟气和舱内辐射，因此结果受二次火灾位置与持续性控制。

该对象当前只建模为不可燃的3 mm铝合金6061外壳，没有内部电路板、线缆或聚合物部件，也没有自身HRRPUA。因此外部脉冲结束后只能依靠邻近火灾继续加热。当前判定反映的是铝壳表面热毁伤代理，不等同于真实内部电子器件一定未发生功能失效。

##### H4：铝合金6061外壳

当前峰值为 **158.4 C**，重度判据为 **400 C 持续 5 s**，实际最长连续时间为 **0.0 s**。

单位面积热容量为 `rho*cp*d = 2700*896*0.003 = 7258 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `275.8 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **4/10** 个对应正外部热流面，登记峰值为 **118 kW/m2**。标称 Q=50 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值仍低于重度阈值 **241.6 C**。对象只有部分监测面直接受照，未受照部分及内部导热共同削弱局部脉冲升温，脉冲后散热又使温度回落。

该对象当前只建模为不可燃的3 mm铝合金6061外壳，没有内部电路板、线缆或聚合物部件，也没有自身HRRPUA。因此外部脉冲结束后只能依靠邻近火灾继续加热。当前判定反映的是铝壳表面热毁伤代理，不等同于真实内部电子器件一定未发生功能失效。

##### H5：铝合金6061外壳

当前峰值为 **62.7 C**，重度判据为 **200 C 持续 180 s**，实际最长连续时间为 **0.0 s**。
...
峰值仍低于重度阈值 **137.3 C**。对象只有部分监测面直接受照，未受照部分及内部导热共同削弱局部脉冲升温，脉冲后散热又使温度回落。
...
状态：**实时快照（运行中）**；数据截止 **532.5 s**；严格重度毁伤 **11/17**；PDF毁伤树整机等级 **重度**。

| 材料/设备 | 毁伤等级 | 最高温度（C） | 重度阈值连续时间（s） | 重度要求（s） | 判定 |
|---|---|---:|---:|---:|---|
| RADM | 重度 | 1002.4 | 531.0 | 180.0 | 达到重度 |
| WINS | 重度 | 1216.7 | 531.0 | 8.0 | 达到重度 |
| BED | 中度 | 526.7 | 4.5 | 5.0 | 峰值或持续时间不足 |
| CURT | 重度 | 1036.2 | 46.5 | 5.0 | 达到重度 |
| U4 | 重度 | 487.7 | 115.5 | 5.0 | 达到重度 |
| SEAT | 重度 | 2726.8 | 531.0 | 5.0 | 达到重度 |
| AL2024 | 重度 | 851.0 | 528.0 | 60.0 | 达到重度 |
| AL5052 | 重度 | 708.3 | 408.0 | 60.0 | 达到重度 |
| AL7075 | 重度 | 652.6 | 393.0 | 60.0 | 达到重度 |
| O2TANK | 重度 | 459.7 | 102.0 | 60.0 | 达到重度 |
| H1 | 未毁伤 | 251.7 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H2 | 轻度 | 277.2 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H3 | 重度 | 631.1 | 334.5 | 5.0 | 达到重度 |
| H4 | 重度 | 556.5 | 316.5 | 5.0 | 达到重度 |
| H5 | 轻度 | 201.5 | 3.0 | 180.0 | 峰值或持续时间不足 |
| H6 | 未毁伤 | 396.3 | 0.0 | 180.0 | 峰值或持续时间不足 |
| H7 | 中度 | 376.9 | 0.0 | 5.0 | 峰值或持续时间不足 |
...
当前峰值为 **201.5 C**，重度判据为 **200 C 持续 180 s**，实际最长连续时间为 **3.0 s**。

单位面积热容量为 `rho*cp*d = 2700*896*0.003 = 7258 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `130.6 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **3/6** 个对应正外部热流面，登记峰值为 **2471 kW/m2**。标称 Q=200 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值已经超过重度温度阈值，但还缺少 **177.0 s** 连续保持时间。这属于持续时间不足，而不是峰值温度不足；后续若温度重新升高并连续越过阈值，等级仍可能改变。

该对象当前只建模为不可燃的3 mm铝合金6061外壳，没有内部电路板、线缆或聚合物部件，也没有自身HRRPUA。因此外部脉冲结束后只能依靠邻近火灾继续加热。当前判定反映的是铝壳表面热毁伤代理，不等同于真实内部电子器件一定未发生功能失效。

##### H6：PVC塑料

当前峰值为 **396.3 C**，重度判据为 **400 C 持续 180 s**，实际最长连续时间为 **0.0 s**。

单位面积热容量为 `rho*cp*d = 1449*840*0.001 = 1217 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `46.3 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **0/8** 个对应正外部热流面，登记峰值为 **0 kW/m2**。标称 Q=200 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值仍低于重度阈值 **3.7 C**，且有效探针没有直接受照面。其升温主要来自邻近火焰、热烟气和舱内辐射，因此结果受二次火灾位置与持续性控制。

该表面设置 `IGNITION_TEMPERATURE=468.6 C`、`HRRPUA=259 kW/m2`。当前峰值低于点燃温度 **72.3 C**，因此尚不能依靠自身燃烧维持升温；提高HRRPUA本身在未点燃前不会生效。

##### H7：CR橡胶

当前峰值为 **376.9 C**，重度判据为 **400 C 持续 5 s**，实际最长连续时间为 **0.0 s**。

单位面积热容量为 `rho*cp*d = 1500*1120*0.002 = 3360 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `127.7 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **0/3** 个对应正外部热流面，登记峰值为 **0 kW/m2**。标称 Q=200 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值仍低于重度阈值 **23.1 C**，且有效探针没有直接受照面。其升温主要来自邻近火焰、热烟气和舱内辐射，因此结果受二次火灾位置与持续性控制。

该表面设置 `IGNITION_TEMPERATURE=410 C`、`HRRPUA=458 kW/m2`。当前峰值低于点燃温度 **33.1 C**，因此尚不能依靠自身燃烧维持升温；提高HRRPUA本身在未点燃前不会生效。

### Q=400 J/cm2

状态：**实时快照（运行中）**；数据截止 **511.5 s**；严格重度毁伤 **14/17**；PDF毁伤树整机等级 **重度**。

| 材料/设备 | 毁伤等级 | 最高温度（C） | 重度阈值连续时间（s） | 重度要求（s） | 判定 |
|---|---|---:|---:|---:|---|
| RADM | 重度 | 1532.6 | 510.0 | 180.0 | 达到重度 |
| WINS | 重度 | 1706.5 | 510.0 | 8.0 | 达到重度 |
| BED | 重度 | 672.6 | 16.5 | 5.0 | 达到重度 |
| CURT | 重度 | 1530.1 | 13.5 | 5.0 | 达到重度 |
| U4 | 重度 | 487.0 | 109.5 | 5.0 | 达到重度 |
| SEAT | 重度 | 2726.8 | 510.0 | 5.0 | 达到重度 |
| AL2024 | 重度 | 860.3 | 510.0 | 60.0 | 达到重度 |
| AL5052 | 重度 | 695.9 | 396.0 | 60.0 | 达到重度 |
| AL7075 | 重度 | 682.4 | 376.5 | 60.0 | 达到重度 |
| O2TANK | 重度 | 462.4 | 112.5 | 60.0 | 达到重度 |
| H1 | 重度 | 468.0 | 19.5 | 5.0 | 达到重度 |
| H2 | 重度 | 489.7 | 27.0 | 5.0 | 达到重度 |
| H3 | 重度 | 630.7 | 316.5 | 5.0 | 达到重度 |
| H4 | 重度 | 555.6 | 307.5 | 5.0 | 达到重度 |
| H5 | 轻度 | 203.9 | 9.0 | 180.0 | 峰值或持续时间不足 |
| H6 | 未毁伤 | 403.4 | 52.5 | 180.0 | 峰值或持续时间不足 |
| H7 | 中度 | 377.1 | 0.0 | 5.0 | 峰值或持续时间不足 |
...
当前峰值为 **203.9 C**，重度判据为 **200 C 持续 180 s**，实际最长连续时间为 **9.0 s**。

单位面积热容量为 `rho*cp*d = 2700*896*0.003 = 7258 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `130.6 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **3/6** 个对应正外部热流面，登记峰值为 **4942 kW/m2**。标称 Q=400 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值已经超过重度温度阈值，但还缺少 **171.0 s** 连续保持时间。这属于持续时间不足，而不是峰值温度不足；后续若温度重新升高并连续越过阈值，等级仍可能改变。

该对象当前只建模为不可燃的3 mm铝合金6061外壳，没有内部电路板、线缆或聚合物部件，也没有自身HRRPUA。因此外部脉冲结束后只能依靠邻近火灾继续加热。当前判定反映的是铝壳表面热毁伤代理，不等同于真实内部电子器件一定未发生功能失效。

##### H6：PVC塑料

当前峰值为 **403.4 C**，重度判据为 **400 C 持续 180 s**，实际最长连续时间为 **52.5 s**。

单位面积热容量为 `rho*cp*d = 1449*840*0.001 = 1217 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `46.3 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **0/8** 个对应正外部热流面，登记峰值为 **0 kW/m2**。标称 Q=400 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值已经超过重度温度阈值，但还缺少 **127.5 s** 连续保持时间。这属于持续时间不足，而不是峰值温度不足；后续若温度重新升高并连续越过阈值，等级仍可能改变。

该表面设置 `IGNITION_TEMPERATURE=468.6 C`、`HRRPUA=259 kW/m2`。当前峰值低于点燃温度 **65.2 C**，因此尚不能依靠自身燃烧维持升温；提高HRRPUA本身在未点燃前不会生效。

##### H7：CR橡胶

当前峰值为 **377.1 C**，重度判据为 **400 C 持续 5 s**，实际最长连续时间为 **0.0 s**。

单位面积热容量为 `rho*cp*d = 1500*1120*0.002 = 3360 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `127.7 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **0/3** 个对应正外部热流面，登记峰值为 **0 kW/m2**。标称 Q=400 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值仍低于重度阈值 **22.9 C**，且有效探针没有直接受照面。其升温主要来自邻近火焰、热烟气和舱内辐射，因此结果受二次火灾位置与持续性控制。

该表面设置 `IGNITION_TEMPERATURE=410 C`、`HRRPUA=458 kW/m2`。当前峰值低于点燃温度 **32.9 C**，因此尚不能依靠自身燃烧维持升温；提高HRRPUA本身在未点燃前不会生效。
