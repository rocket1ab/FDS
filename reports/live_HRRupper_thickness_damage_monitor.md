...
- 本次检查时间：**2026-07-22T22:18:00+08:00**
...
|---|---|---|---:|---:|---|
| node01 | Q50 | 实时快照（运行中） | 198.0 / 1500 | 2/17 | 重度 |
| node04 | Q400 -> Q300 | 实时快照（运行中） | 718.5 / 1500 | 17/17 | 重度 |
| node05 | Q200 -> Q100 | 实时快照（运行中） | 745.5 / 1500 | 14/17 | 重度 |
...
状态：**实时快照（运行中）**；数据截止 **198.0 s**；严格重度毁伤 **2/17**；PDF毁伤树整机等级 **重度**。

| 材料/设备 | 毁伤等级 | 最高温度（C） | 重度阈值连续时间（s） | 重度要求（s） | 判定 |
|---|---|---:|---:|---:|---|
| RADM | 未毁伤 | 803.5 | 178.5 | 180.0 | 峰值或持续时间不足 |
| WINS | 重度 | 950.3 | 196.5 | 8.0 | 达到重度 |
| BED | 未毁伤 | 267.1 | 0.0 | 5.0 | 峰值或持续时间不足 |
| CURT | 中度 | 354.2 | 0.0 | 5.0 | 峰值或持续时间不足 |
| U4 | 未毁伤 | 147.3 | 0.0 | 5.0 | 峰值或持续时间不足 |
| SEAT | 重度 | 2726.8 | 196.5 | 5.0 | 达到重度 |
| AL2024 | 未毁伤 | 842.5 | 175.5 | 180.0 | 峰值或持续时间不足 |
| AL5052 | 未毁伤 | 482.3 | 39.0 | 180.0 | 峰值或持续时间不足 |
| AL7075 | 未毁伤 | 325.1 | 0.0 | 180.0 | 峰值或持续时间不足 |
| O2TANK | 未毁伤 | 151.6 | 0.0 | 180.0 | 峰值或持续时间不足 |
| H1 | 未毁伤 | 78.3 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H2 | 未毁伤 | 78.4 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H3 | 未毁伤 | 395.8 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H4 | 未毁伤 | 292.1 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H5 | 未毁伤 | 70.1 | 0.0 | 180.0 | 峰值或持续时间不足 |
...
当前峰值为 **803.5 C**，重度判据为 **400 C 持续 180 s**，实际最长连续时间为 **178.5 s**。
...
峰值已经超过重度温度阈值，但还缺少 **1.5 s** 连续保持时间。这属于持续时间不足，而不是峰值温度不足；后续若温度重新升高并连续越过阈值，等级仍可能改变。
...
当前峰值为 **354.2 C**，重度判据为 **500 C 持续 5 s**，实际最长连续时间为 **0.0 s**。
...
峰值仍低于重度阈值 **145.8 C**。虽然监测面直接受照，但实际吸收能量受投影、表面换热和材料热惯性限制，且后续二次火灾没有提供足够持续加热。

该表面设置 `IGNITION_TEMPERATURE=250 C`、`HRRPUA=324 kW/m2`，峰值已越过点燃温度。若仍未满足重度判据，重点应检查点燃后的有效燃烧面积、持续热释放、邻近表面的热反馈以及探针是否覆盖持续热点。

##### U4：环氧玻璃纤维

当前峰值为 **147.3 C**，重度判据为 **400 C 持续 5 s**，实际最长连续时间为 **0.0 s**。
...
峰值仍低于重度阈值 **252.7 C**，且有效探针没有直接受照面。其升温主要来自邻近火焰、热烟气和舱内辐射，因此结果受二次火灾位置与持续性控制。

该表面设置 `IGNITION_TEMPERATURE=350 C`、`HRRPUA=840 kW/m2`。当前峰值低于点燃温度 **202.7 C**，因此尚不能依靠自身燃烧维持升温；提高HRRPUA本身在未点燃前不会生效。

##### AL2024：铝合金2024

当前峰值为 **842.5 C**，重度判据为 **400 C 持续 180 s**，实际最长连续时间为 **175.5 s**。

单位面积热容量为 `rho*cp*d = 2780*875*0.002 = 4865 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `184.9 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **5/10** 个对应正外部热流面，登记峰值为 **735 kW/m2**。标称 Q=50 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值已经超过重度温度阈值，但还缺少 **4.5 s** 连续保持时间。这属于持续时间不足，而不是峰值温度不足；后续若温度重新升高并连续越过阈值，等级仍可能改变。

##### AL5052：铝合金5052

当前峰值为 **482.3 C**，重度判据为 **400 C 持续 180 s**，实际最长连续时间为 **39.0 s**。
...
峰值已经超过重度温度阈值，但还缺少 **141.0 s** 连续保持时间。这属于持续时间不足，而不是峰值温度不足；后续若温度重新升高并连续越过阈值，等级仍可能改变。

##### AL7075：铝合金7075

当前峰值为 **325.1 C**，重度判据为 **400 C 持续 180 s**，实际最长连续时间为 **0.0 s**。
...
当前峰值为 **151.6 C**，重度判据为 **400 C 持续 180 s**，实际最长连续时间为 **0.0 s**。
...
峰值仍低于重度阈值 **248.4 C**。对象只有部分监测面直接受照，未受照部分及内部导热共同削弱局部脉冲升温，脉冲后散热又使温度回落。
...
当前峰值为 **395.8 C**，重度判据为 **400 C 持续 5 s**，实际最长连续时间为 **0.0 s**。

单位面积热容量为 `rho*cp*d = 2700*896*0.003 = 7258 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `275.8 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **0/14** 个对应正外部热流面，登记峰值为 **0 kW/m2**。标称 Q=50 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值仍低于重度阈值 **4.2 C**，且有效探针没有直接受照面。其升温主要来自邻近火焰、热烟气和舱内辐射，因此结果受二次火灾位置与持续性控制。

该对象当前只建模为不可燃的3 mm铝合金6061外壳，没有内部电路板、线缆或聚合物部件，也没有自身HRRPUA。因此外部脉冲结束后只能依靠邻近火灾继续加热。当前判定反映的是铝壳表面热毁伤代理，不等同于真实内部电子器件一定未发生功能失效。

##### H4：铝合金6061外壳

当前峰值为 **292.1 C**，重度判据为 **400 C 持续 5 s**，实际最长连续时间为 **0.0 s**。

单位面积热容量为 `rho*cp*d = 2700*896*0.003 = 7258 J/(m2*K)`；从 20 C 升至重度阈值所需的理想显热约为 `275.8 J/cm2`。该值忽略反射、角度投影、向实体内部导热、对流、辐射散热和热解，只用于解释热惯性，不能直接当作光冲量阈值。

有效温度探针中有 **4/10** 个对应正外部热流面，登记峰值为 **118 kW/m2**。标称 Q=50 J/cm2 是入射平面光冲量，不等于该对象实际吸收的光冲量。

峰值仍低于重度阈值 **107.9 C**。对象只有部分监测面直接受照，未受照部分及内部导热共同削弱局部脉冲升温，脉冲后散热又使温度回落。

该对象当前只建模为不可燃的3 mm铝合金6061外壳，没有内部电路板、线缆或聚合物部件，也没有自身HRRPUA。因此外部脉冲结束后只能依靠邻近火灾继续加热。当前判定反映的是铝壳表面热毁伤代理，不等同于真实内部电子器件一定未发生功能失效。

##### H5：铝合金6061外壳

当前峰值为 **70.1 C**，重度判据为 **200 C 持续 180 s**，实际最长连续时间为 **0.0 s**。
...
峰值仍低于重度阈值 **129.9 C**。对象只有部分监测面直接受照，未受照部分及内部导热共同削弱局部脉冲升温，脉冲后散热又使温度回落。
...
状态：**实时快照（运行中）**；数据截止 **745.5 s**；严格重度毁伤 **14/17**；PDF毁伤树整机等级 **重度**。

| 材料/设备 | 毁伤等级 | 最高温度（C） | 重度阈值连续时间（s） | 重度要求（s） | 判定 |
|---|---|---:|---:|---:|---|
| RADM | 重度 | 1002.4 | 744.0 | 180.0 | 达到重度 |
| WINS | 重度 | 1216.7 | 744.0 | 8.0 | 达到重度 |
| BED | 中度 | 526.7 | 4.5 | 5.0 | 峰值或持续时间不足 |
| CURT | 重度 | 1036.2 | 259.5 | 5.0 | 达到重度 |
| U4 | 重度 | 603.7 | 328.5 | 5.0 | 达到重度 |
| SEAT | 重度 | 2726.8 | 744.0 | 5.0 | 达到重度 |
| AL2024 | 重度 | 851.0 | 741.0 | 180.0 | 达到重度 |
| AL5052 | 重度 | 708.3 | 621.0 | 180.0 | 达到重度 |
| AL7075 | 重度 | 684.8 | 606.0 | 180.0 | 达到重度 |
| O2TANK | 重度 | 513.3 | 315.0 | 180.0 | 达到重度 |
| H1 | 轻度 | 251.7 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H2 | 轻度 | 277.2 | 0.0 | 5.0 | 峰值或持续时间不足 |
| H3 | 重度 | 659.3 | 547.5 | 5.0 | 达到重度 |
| H4 | 重度 | 587.7 | 529.5 | 5.0 | 达到重度 |
| H5 | 重度 | 259.4 | 216.0 | 180.0 | 达到重度 |
| H6 | 重度 | 434.7 | 196.5 | 180.0 | 达到重度 |
| H7 | 重度 | 401.6 | 19.5 | 5.0 | 达到重度 |
...
状态：**实时快照（运行中）**；数据截止 **718.5 s**；严格重度毁伤 **17/17**；PDF毁伤树整机等级 **重度**。

| 材料/设备 | 毁伤等级 | 最高温度（C） | 重度阈值连续时间（s） | 重度要求（s） | 判定 |
|---|---|---:|---:|---:|---|
| RADM | 重度 | 1532.6 | 717.0 | 180.0 | 达到重度 |
| WINS | 重度 | 1706.5 | 717.0 | 8.0 | 达到重度 |
| BED | 重度 | 672.6 | 16.5 | 5.0 | 达到重度 |
| CURT | 重度 | 1530.1 | 217.5 | 5.0 | 达到重度 |
| U4 | 重度 | 599.0 | 316.5 | 5.0 | 达到重度 |
| SEAT | 重度 | 2726.8 | 717.0 | 5.0 | 达到重度 |
| AL2024 | 重度 | 860.3 | 717.0 | 180.0 | 达到重度 |
| AL5052 | 重度 | 695.9 | 603.0 | 180.0 | 达到重度 |
| AL7075 | 重度 | 682.4 | 583.5 | 180.0 | 达到重度 |
| O2TANK | 重度 | 514.2 | 319.5 | 180.0 | 达到重度 |
| H1 | 重度 | 468.0 | 19.5 | 5.0 | 达到重度 |
| H2 | 重度 | 489.7 | 27.0 | 5.0 | 达到重度 |
| H3 | 重度 | 659.1 | 523.5 | 5.0 | 达到重度 |
| H4 | 重度 | 585.8 | 514.5 | 5.0 | 达到重度 |
| H5 | 重度 | 260.0 | 216.0 | 180.0 | 达到重度 |
| H6 | 重度 | 435.7 | 259.5 | 180.0 | 达到重度 |
| H7 | 重度 | 402.6 | 21.0 | 5.0 | 达到重度 |
