...
def methodology_markdown(heading_level=2):
    ...
    return [
        "", f"{h} 毁伤树评估原则", "",
        f"{h}# PDF 原始毁伤树来源与结构", "",
        "本项目毁伤树的基础来源为用户提供的毁伤标准 PDF。PDF 第 6.2 节将毁伤事件区分为底事件、中间事件和顶事件，并提出从飞机目标向系统、功能单元和子系统逐级分解；第 6.3 节结合 DMECA 结果，分别构建重度、中度和轻度毁伤树。以下图片为 PDF 表13至表15的原始页面摘录，用于保持标准来源的可追溯性。", "",
        "**PDF 表13：重度毁伤等级毁伤树原图摘录**", "",
        "![PDF 原始重度毁伤树](figures/pdf_original_severe_damage_tree.png)", "",
        "**PDF 表14：中度毁伤等级毁伤树原图摘录**", "",
        "![PDF 原始中度毁伤树](figures/pdf_original_moderate_damage_tree.png)", "",
        "**PDF 表15：轻度毁伤等级毁伤树原图摘录**", "",
        "![PDF 原始轻度毁伤树](figures/pdf_original_mild_damage_tree.png)", "",
        "由于 PDF 原图跨页排版，下面将表13的重度毁伤树重新绘制为一张清晰图。重绘仅改善可读性，不改变原图的目标、系统和底事件关系。", "",
        "![PDF 重度毁伤树清晰重绘](figures/pdf_severe_damage_tree_redrawn.svg)", "",
        "PDF 原始树的顶层结构为“飞机目标、机体结构系统、航电系统、电力系统、座舱系统”。表10和表11规定，系统及飞机目标的等级由下一级事件中的最高毁伤等级确定。重度毁伤树因此表现为至少一个关键系统或主要要害部位达到重度即可向上触发；中度和轻度则还要求不存在更高等级事件。", "",
        "PDF 第 6.2 节给出的建树原则可归纳为：第一，自上而下按功能划分飞机目标和系统；第二，把系统毁伤作为次顶事件，把功能单元毁伤作为中间事件，把子系统或要害部位毁伤作为底事件；第三，根据毁伤等级和 DMECA 影响确定要害部位，并用适当逻辑门连接；第四，建树前必须掌握飞机结构组成和功能属性。", "",
