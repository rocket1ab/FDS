...
def methodology_markdown(heading_level=2):
    ...
    return [
        ...
        "机体结构系统以机头结构为主要要害节点。机头结构达到重度时系统直接为重度；达到中度且不存在重度时系统为中度；仅达到轻度且不存在更高等级时系统为轻度。当前模型进一步使用 RADM、AL2024 和 AL7075 提供机头、蒙皮和框架的热响应证据。", "",
        "航电系统的逻辑最需要区分主要和次要节点。PDF 重度毁伤树中，雷达和导航是能够直接触发航电系统重度毁伤的主要节点。通信、光电子和电子对抗作为次要节点时，其重度毁伤可在没有主要节点重度的条件下触发系统中度，而不能直接触发系统重度。当前模型将 H1、H2 作为主要航电节点，将 H4 作为次要航电节点；其中 H2 是项目扩展映射，需要在报告中保留这一限制。", "",
